...
// =====================================================================
// phi_d2d_lite_v2.v  —  CORRECTED Phi mesh-lite bi-port serial adapter.
// Apache-2.0
//
// FIX (2026-06): the shipped phi_d2d_lite RX required the START bit to read LOW
// across TWO cycles (RX_IDLE detect + RX_DETECT_START confirm), but its own TX
// emits a ONE-cycle START. So RX consumed the first DATA bit as the start-confirm,
// misaligning the whole word by one bit -> parity never matched -> the receive
// path NEVER delivered a packet (verified: TX->RX loopback delivers nothing for
// any input; an externally-driven 2-cycle start delivers correctly). Two Phi dies
// wired together (A.e_tx -> B.e_rx) could not communicate. v2 makes RX treat the
// START as one cycle (RX_IDLE -> RX_DATA directly), so the first RX_DATA cycle
// samples D0. Verified: TX->RX loopback round-trips every test packet.
//
// Latent on the current single-die TTSKY26b shuttle (no second die to talk to);
// this is the correction for the intended multi-die mesh. Frozen source untouched.
//
module phi_d2d_lite_v2 (
    input  wire        clk,
    input  wire        rst_n,
    input  wire        ena,
