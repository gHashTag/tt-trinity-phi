...
// t27/rtl_gen/gf24_add.v
// GoldenFloat24 Addition Unit -- [S(1) | E(9) | M(14)], bias 2^(E-1)-1.
...
// the 14-bit mantissa is rounded to nearest, ties-to-even. The running exponent is
...
module gf24_add (
    ...
    reg [8:0] big_exp, small_exp;
    reg [13:0] big_m, small_m;
    reg [17:0] big_ext, small_ext, shifted;   // {1, mant, 3 guard bits}
    reg [9:0]  shamt;
    reg            sticky_shift;
    reg [18:0] sum_m;                          // room for add carry
    reg signed [10:0] rexp;
    reg [13:0]  mant_out;
    reg [14:0]   mant_rounded;                    // M+1 bits, catches overflow
    ...
    always @(*) begin
        ...
        if (is_nan_a || is_nan_b)
            ...
            result = sign_a ? 24'hFFC000 : 24'h7FC000;
        else if (is_inf_b)
            result = sign_b ? 24'hFFC000 : 24'h7FC000;
            ...
            if (shamt >= 18)
                ...
                sticky_shift = |(small_ext & ((18'd1 << shamt) - 18'd1));
            end
            ...
            if (sum_m == 0) begin
                ...
                if (sum_m[18]) begin
                    ...
                    rexp = rexp - 11'sd4;
                    mant_out = {sum_m[12:0], 1'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[12]) begin
                    rexp = rexp - 11'sd5;
                    mant_out = {sum_m[11:0], 2'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[11]) begin
                    rexp = rexp - 11'sd6;
                    mant_out = {sum_m[10:0], 3'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[10]) begin
                    rexp = rexp - 11'sd7;
                    mant_out = {sum_m[9:0], 4'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[9]) begin
                    rexp = rexp - 11'sd8;
                    mant_out = {sum_m[8:0], 5'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[8]) begin
                    rexp = rexp - 11'sd9;
                    mant_out = {sum_m[7:0], 6'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[7]) begin
                    rexp = rexp - 11'sd10;
                    mant_out = {sum_m[6:0], 7'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[6]) begin
                    rexp = rexp - 11'sd11;
                    mant_out = {sum_m[5:0], 8'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[5]) begin
                    rexp = rexp - 11'sd12;
                    mant_out = {sum_m[4:0], 9'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[4]) begin
                    rexp = rexp - 11'sd13;
                    mant_out = {sum_m[3:0], 10'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[3]) begin
                    rexp = rexp - 11'sd14;
                    mant_out = {sum_m[2:0], 11'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[2]) begin
                    rexp = rexp - 11'sd15;
                    mant_out = {sum_m[1:0], 12'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[1]) begin
                    rexp = rexp - 11'sd16;
                    mant_out = {sum_m[0:0], 13'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else begin
                    mant_out = 14'd0;
                ...
                mant_rounded = {1'b0, mant_out} + (round_up ? 15'd1 : 15'd0);
                if (mant_rounded[14]) begin
                    rexp     = rexp + 11'sd1;
                    mant_out = 14'd0;
                end else begin
                    mant_out = mant_rounded[13:0];
                end

                if (rexp < 0)
                    result = result_sign ? 24'h800000 : 24'h0;
                else if (rexp >= EXP_MAX_S)
                    result = result_sign ? 24'hFFC000 : 24'h7FC000;
                else if (rexp == 0 && mant_out == 14'd0)
                    result = result_sign ? 24'h800000 : 24'h0;
                else
                    result = {result_sign, rexp[8:0], mant_out};
            end
        end
    end
