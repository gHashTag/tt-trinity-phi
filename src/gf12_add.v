...
// GoldenFloat12 Addition Unit -- [S(1) | E(4) | M(7)], bias 2^(E-1)-1.
...
// the 7-bit mantissa is rounded to nearest, ties-to-even. The running exponent is
...
module gf12_add (
    ...
    reg [3:0] big_exp, small_exp;
    reg [6:0] big_m, small_m;
    reg [10:0] big_ext, small_ext, shifted;   // {1, mant, 3 guard bits}
    reg [4:0]  shamt;
    reg            sticky_shift;
    reg [11:0] sum_m;                          // room for add carry
    reg signed [5:0] rexp;
    reg [6:0]  mant_out;
    reg [7:0]   mant_rounded;                    // M+1 bits, catches overflow
    ...
    always @(*) begin
        ...
        if (is_nan_a || is_nan_b)
            ...
            if (shamt >= 11)
                ...
                sticky_shift = |(small_ext & ((11'd1 << shamt) - 11'd1));
            end
            ...
            if (sum_m == 0) begin
                ...
                if (sum_m[11]) begin
                    ...
                    rexp = rexp - 6'sd4;
                    mant_out = {sum_m[5:0], 1'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[5]) begin
                    rexp = rexp - 6'sd5;
                    mant_out = {sum_m[4:0], 2'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[4]) begin
                    rexp = rexp - 6'sd6;
                    mant_out = {sum_m[3:0], 3'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[3]) begin
                    rexp = rexp - 6'sd7;
                    mant_out = {sum_m[2:0], 4'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[2]) begin
                    rexp = rexp - 6'sd8;
                    mant_out = {sum_m[1:0], 5'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else if (sum_m[1]) begin
                    rexp = rexp - 6'sd9;
                    mant_out = {sum_m[0:0], 6'd0};
                    guard = 1'b0; round_b = 1'b0; sticky = sticky_shift;
                end
                else begin
                    mant_out = 7'd0;
                ...
                mant_rounded = {1'b0, mant_out} + (round_up ? 8'd1 : 8'd0);
                if (mant_rounded[7]) begin
                    rexp     = rexp + 6'sd1;
                    mant_out = 7'd0;
                end else begin
                    mant_out = mant_rounded[6:0];
                end
